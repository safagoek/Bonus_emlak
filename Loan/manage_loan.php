...
<div class="container-fluid">
	...
	<form action="" id="loan-application">
		...
		<div class="row">
			<div class="col-md-6">
				<label class="control-label">Kiracı</label>
				<?php
		...
		<div class="row">
			<div class="col-md-6">
				<label class="control-label">Kira Planı</label>
				<?php
		...
		<div class="form-group col-md-6">
			<label class="control-label">Toplam Kira Bedeli</label>
		...
		<div class="form-group col-md-2 offset-md-2 .justify-content-center">
			...
			<button class="btn btn-primary btn-sm btn-block align-self-end" type="button" id="calculate">Hesapla</button>
		</div>
		</div>
		...
		<div class="row">
			<div class="form-group col-md-6">
				...
				<select class="custom-select browser-default" name="status">
					<option value="0" <?php echo $status == 0 ? "selected" : '' ?>>Onay Bekliyor</option>
					<?php if($status !='4' ): ?>
					<option value="2" <?php echo $status == 2 ? "selected" : '' ?>>Sözleşme Onaylandı</option>
					<?php endif ?>
					<?php if($status =='2' ): ?>
					<option value="3" <?php echo $status == 3 ? "selected" : '' ?>>Sözleşme Tamamlandı</option>
					<?php endif ?>
					<?php if($status !='2' ): ?>
					<option value="4" <?php echo $status == 4 ? "selected" : '' ?>>Reddedildi</option>
					<?php endif ?>
				</select>
			</div>
		</div>
		...
		<div id="row-field">
			<div class="row ">
				<div class="col-md-12 text-center">
					...
					<button class="btn btn-secondary btn-sm" type="button" data-dismiss="modal">İptal Et</button>
				</div>
			</div>
		</div>
	...
	$('#loan-application').submit(function(e){
		...
		$.ajax({
			...
			success:function(resp){
				if(resp ==1 ){
					$('.modal').modal('hide')
					alert_toast("Sözleşme Başarıyla Kaydedildi.","success")
					setTimeout(function(){
						location.reload();
					},1500)
				}
