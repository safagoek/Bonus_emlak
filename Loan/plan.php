				...
				<div class="card">
					...
					<div class="card-body">
							...
							<div class="form-group">
								<label class="control-label">Plan (Aylık)</label>
								<input type="number" name="months" id="" class="form-control text-right">
							</div>
							<div class="form-group">
								<label class="control-label">Yıllık Artış Miktarı (Yüzde)</label>
							...
							<div class="form-group">
								<label class="control-label">Aylık Gecikme Faizi (Yüzde)</label>
								<div class="input-group">
					...
					<div class="card-footer">
						<div class="row">
							<div class="col-md-12">
								...
								<button class="btn btn-sm btn-default col-sm-3" type="button" onclick="_reset()"> İptal</button>
							</div>
						</div>
					</div>
				</div>
						...
						<table class="table table-bordered table-hover">
							...
							<tbody>
								...
								<tr>
									...
									<td class="">
										 <p>Yıllar/Aylar: <b><?php echo $months ?></b></p>
										 <p><small>Yıllık Artış Oranı: <b><?php echo $row['interest_percentage']."%" ?></b></small></p>
										 <p><small>Aylık Gecikme Faizi: <b><?php echo $row['penalty_rate']."%" ?></b></small></p>
									</td>
									<td class="text-center">
										<button class="btn btn-sm btn-primary edit_plan" type="button" data-id="<?php echo $row['id'] ?>" data-months="<?php echo $row['months'] ?>" data-interest_percentage="<?php echo $row['interest_percentage'] ?>" >Düzenle</button>
										<button class="btn btn-sm btn-danger delete_plan" type="button" data-id="<?php echo $row['id'] ?>">Sil</button>
									</td>
								</tr>
								<?php endwhile; ?>
							</tbody>
...
<script>
	...
	$('#manage-plan').submit(function(e){
		...
		$.ajax({
			...
			success:function(resp){
				if(resp==1){
					alert_toast("Plan Başarıyla Eklendi",'success')
					setTimeout(function(){
						location.reload()
					},1500)
					...
					alert_toast("Plan Başarıyla Güncellendi",'success')
					setTimeout(function(){
						location.reload()
					},1500)
	...
	$('.delete_plan').click(function(){
		_conf("Bu Planı Silmek İstediğinize Emin Misiniz?","delete_plan",[$(this).attr('data-id')])
	})
	...
	function delete_plan($id){
		...
		$.ajax({
			...
			success:function(resp){
				if(resp==1){
					alert_toast("Plan Başarıyla Silindi",'success')
					setTimeout(function(){
						location.reload()
					},1500)
